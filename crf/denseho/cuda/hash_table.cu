...
// Device pointer point to device memory
...
// Host pointer pointing to device memory
float *host_table_values;
signed short *host_table_keys;
int *host_table_entries;
unsigned int host_table_capacity;
signed short *host_table_zeros;
char *host_table_rank;
...
void createHashTable(int capacity) {
    ...
    allocateCudaMemory((void**)&host_table_values, capacity*vd*sizeof(float));
    CUDA_SAFE_CALL(cudaMemset((void *)host_table_values, 0, capacity*vd*sizeof(float)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(table_values,
                      &host_table_values,
				      sizeof(float *)));       
    

    allocateCudaMemory((void **)&host_table_entries, capacity*2*sizeof(int));
    CUDA_SAFE_CALL(cudaMemset((void *)host_table_entries, -1, capacity * 2 * sizeof(int)));
    ...
    allocateCudaMemory((void **)&host_table_keys, capacity*kd*sizeof(signed short));
    CUDA_SAFE_CALL(cudaMemset((void *)host_table_keys, 0, capacity*kd*sizeof(signed short)));
...
template <int vd> static void resetHashTable() {
    CUDA_SAFE_CALL(cudaMemset(host_table_values, 0, host_table_capacity*vd*sizeof(float)));
}

static void destroyHashTable() {
#ifndef LINEAR_D_MEMORY
    CUDA_SAFE_CALL(cudaFree(host_table_keys));
#endif
    CUDA_SAFE_CALL(cudaFree(host_table_values));
    CUDA_SAFE_CALL(cudaFree(host_table_entries));
}
...
static float* swapHashTableValues(float *newValues) {
    ...
    CUDA_SAFE_CALL(cudaMemcpyFromSymbol(&oldValues,   // copy to host
					table_values,
					sizeof(float *)));

    //CUDA_SAFE_CALL(cudaGetSymbolAddress((void**)&oldValues, table_values));
