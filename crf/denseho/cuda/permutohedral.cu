...
struct MatrixEntry {
    int index;
    float weight;   // barycentric weight
};
...
__global__ static void createMatrix(const int w, const int h, 
    ...
    if (!outOfBounds) {
	    ...
	    for (int i = 0; i <= pd; i++) {
            ...
            if (pd - myRank[i] >= 0 && pd-myRank[i] < pd+2)    // SH: range check?
	            myBarycentric[pd-myRank[i]] += delta;

            if (pd + 1 - myRank[i] >= 0 && pd + 1 - myRank[i] < pd+2) // SH: range check?
...
template<int pd, int vd>
__global__ static void splatCache(const int w, const int h, const float *values, MatrixEntry *matrix
) {
    ...
    __shared__ float sharedValues[BLOCK_SIZE*(vd+1)];  // last value in each block is used to store the total weight
    int myOffset = -1;
...
template<int pd, int vd>
__global__ static void slice(const int w, const int h, float *values, const MatrixEntry *matrix) {
    //const int idx = blockIdx.x * BLOCK_SIZE + threadIdx.x;    
...
void filter_(float *im, float *ref, int w, int h, bool accurate) {    
    ...
    dim3 blocks((w-1)/BLOCK_SIZE+1, 1, 1);
    ...
    getLastCudaError("Matrix creation failed\n");
    ...
    getLastCudaError("splat failed\n");

    gettimeofday(t+3, NULL);    

    // blurring
    if (accurate) {
	    static float *newValues = NULL;
        if (newValues == NULL) {
    ...
    // This function writes results to the values array
    blocks.y /= (pd+1);
    slice<pd, vd><<<blocks, blockSize>>>(w, h, values.device, matrix.device);
    getLastCudaError("slice failed\n");	
...
void filter(float *im, float *ref, int pd, int vd, int w, int h, bool accurate) {
    switch (vd*1000 + pd) {
    //case 4004:
    //    filter_<4, 4>(im, ref, w, h, accurate);
    //    break;  // for 16 labels propagation based on pixel position
    case 21002: 
        filter_<21, 2>(im, ref, w, h, accurate); 
        break;  // for 21 labels propagation based on pixel position
    case 21005:
        filter_<21, 5>(im, ref, w, h, accurate);
        break;  // for 21 labels propagation based on pixel position
    case 37003:
        filter_<37, 3>(im, ref, w, h, accurate);
        break;
    case 37006:
        filter_<37, 6>(im, ref, w, h, accurate);
        break;
